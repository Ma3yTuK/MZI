...
function prepareMessage(message, k) { // here message mod k = 0
    ...
    for (i = 0; i + k <= message.length * BITS_IN_BYTE; i += k) {
        ...
        result.push(helpers.shiftRight(block.slice(0, block_size), block_size * BITS_IN_BYTE - k));
    }
...
function unprepareMessage(message, k) { // message mod k = 0
    ...
    for (let i = 0; i < message.length; i++) {
        let block = helpers.shiftRight(new Uint8Array([...helpers.shiftLeft(message[i], message[i].length * BITS_IN_BYTE - k), 0]), i * k % BITS_IN_BYTE);
...
export function decrypt(message, closed_key) {
    ...
    let suffix_point = [helpers.truncateValue(message.slice(-suffix_value_size * 2, -suffix_value_size)), helpers.truncateValue(message.slice(-suffix_value_size))];
    ...
    let sub = elliptic.point_by_scalar(suffix_point, d)[0];

    for (let i = 0; i < prepared_message.length; i++) {
        result[i] = helpers.resizeVal(helpers.valueDiv(helpers.valueSub(helpers.valueSum(helpers.truncateValue(prepared_message[i]), q), sub), q)[1], result_block_size);
    }
