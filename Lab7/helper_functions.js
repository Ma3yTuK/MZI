...
export function resizeVal(value, size) {
    let diff = value.length - Math.ceil(size / BITS_IN_BYTE);

    if (diff < 0) {
        return new Uint8Array([...new Uint8Array(-diff), ...value]);
    }

    return value.slice(diff);
}
...
export function getRealSize(value) {
    let i = 0;
    while(i < value.length * BITS_IN_BYTE && getBit(value, i++) == 0);
    return value.length * BITS_IN_BYTE - i + 1;
}
...
export function valueNEG(value) {
